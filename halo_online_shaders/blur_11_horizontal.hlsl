...
float4 dynamic_blur_horizontal(float2 texcoord)
{	
    ...
    for (int sample_index = 0; sample_index < number_of_taps; sample_index++)
    {
        ...
        sample_texcoord.xy = float2(texcoord.x + location_delta * pixel_size.x, texcoord.y);
...
fast4 default_ps(screen_output IN) : SV_Target
{
    ...
    return dynamic_blur_horizontal(sample);
