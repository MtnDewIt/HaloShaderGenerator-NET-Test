...
#else
	float4 misc_info= 
		float4(sqrt(height_scale_global * wave_height), sqrt(height_scale_global * wave_height_aux),
			0.0f, IN.local_info.y);	// height_scale, height_scale_aux, water_height, water_depth
#endif
...
accum_pixel water_shading(s_water_interpolators INTERPOLATORS)
{
	...
	float depth_water0 = depth_buffer.Load(int3((texcoord_ss0 * float2(depth_width, depth_height)), 0)).r;

	if (depth_water0 > INTERPOLATORS.position.z)
	{
		...
		float4 scene_ldr_texture_color = scene_ldr_texture.t.Load(int3((texcoord_ss0 * float2(scene_ldr_texture_width, scene_ldr_texture_height)), 0));
...
//		{
//			// if depth test failed return current pixel color
//			float4 scene_ldr_texture_color = sample2D(scene_ldr_texture, texcoord_ss0);
//			return convert_to_render_target(scene_ldr_texture_color, true, true, 0.0f);
//		}
//	#endif
#endif
...
#if (! defined(pc)) || (DX_VERSION == 11) || (defined(APPLY_FIXES))
	[branch]
	if ( k_is_camera_underwater )
	{
...
/*
	...
	return convert_to_render_target(float4(to_show, 1.0f), true, true, 0.0f);
#endif
*/
////////////////////////////////////////////////////////////////////////////////////////////////////
