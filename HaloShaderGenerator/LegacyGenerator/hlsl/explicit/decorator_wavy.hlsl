...
VS_DECORATOR_DEFAULT_OUTPUT vs_default(in S_DECORATOR_VERTEX_INPUT input)
{
    ...
    float wave_scale = sin((input.position.z * wave_flow.w) + (wind_data2.w * wave_flow.z) + dot(input.instance_position.xy, wave_flow.xy));
    float wave_depth_scale = saturate(abs(input.position.z));
    input.position.x += instance_movement_factor * wave_depth_scale * wave_scale;
